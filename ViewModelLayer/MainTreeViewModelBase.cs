...
namespace ViewModelLayer
{
    public abstract class MainTreeViewModelAbstract : ViewModelBase, IMainTreeViewModel
    {
                                            ...
                                            EditDialogHandler<ChildDto, string> childTitleEditDialog)
        {
            ...
            RootEditCommand = new RelayCommand<RootVM>(RootEditExecuteAsync, RootEditCanExecute);
            ChildEditCommand = new RelayCommand<ChildVM>(ChildEditExecuteAsync, ChildEditCanExecute);
        ...
        /// <summary>Асинхронный метод сохранения данных корневого элемента.</summary>
        ...
        protected abstract Task<RootDto> RootSaveAsync(RootDto root, string title);

        /// <summary>Асинхронный метод сохранения данных дочернего элемента.</summary>
        /// <param name="root">Исходные Данные отражающие элемент.</param>
        /// <param name="title">Данные для сохранения.</param>
        /// <returns>Новый экземпляр Данных отражающих элемент.</returns>
        protected abstract Task<ChildDto> ChildSaveAsync(ChildDto root, string title);

        private async void RootEditExecuteAsync(RootVM root)
        {
            if (rootTitleEditDialog(root.Data, out string title))
            {
                root.SetData(await RootSaveAsync(root.Data, title));
            }
        }
        ...
        private async void ChildEditExecuteAsync(ChildVM child)
        {
            if (childTitleEditDialog(child.Data, out string title))
            {
                child.SetData(await ChildSaveAsync(child.Data, title));
            }
        }
